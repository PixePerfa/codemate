...
def test_chromadb():
    config = DBConfig(
        name="CHROMA", path="/tmp", collection_name="db_util_test", emb_name="DEFAULT"
    )
    ...
    filter_op = vector_db.filter(
        kvs={"repo_url": "https://github.com/run-llama/llama_index"}
    )
    print("chroma filter op", filter_op)
    assert len(filter_op["documents"]) > 0
    search_op = vector_db.search(
        query=["Framework for developing LLM applications for coding"], top_k=2
    )
