...
class GithubDataLoader:
    ...
    def mock(self):
        mock_list=['AnswerDotAI/fsdp_qlora','facebookresearch/llama-recipes', 'lucidrains/diffusion-policy', 'lucidrains/DALLE2-pytorch', 'lucidrains/imagen-pytorch', 'lucidrains/vit-pytorch',
                   'lucidrains/denoising-diffusion-pytorch']
        return self.search_repos_by_name(mock_list)

    def search_repos_by_name(self, repo_names:List[str], timeout: int=50, **kwargs) -> List[GithubRepoItem]:
        """
        ...
        for name in repo_names:
            ...
            for item in response["items"]:
                ...
                items.append(GithubRepoItem(
                    repo_url=repo, desc=desc, lang=item["language"],
                    name=item['full_name'], owner=item['owner']['login'],
                    stars=item["stargazers_count"], forks=item.get("forks_count", 0)))

        return items
    
    def search_repos_by_topic(self, topics:List[str], timeout: int=50, min_stars: int=1_000, top_k: int=-1, **kwargs) -> List[GithubRepoItem]:
        """
        ...
        for topic in topics:
            ...
            response = response.json()
            for item in response["items"]:
                if item.get("stargazers_count", 0) < min_stars or not item.get("full_name", "") or not item.get("description", "") or not item.get("language", ""):
                    continue
                repo = f'https://github.com/{item["full_name"]}'
                ...
                items.append(GithubRepoItem(
                    repo_url=repo, desc=trans_desc, lang=item["language"],
                    name=item['full_name'], owner=item['owner']['login'],
                    stars=item["stargazers_count"], forks=item.get("forks_count", 0)))
    ...
    def get_trending_repos(self, since:Literal["daily", "weekly", "monthly"] ="monthly", timeout:int=50, top_k:int=-1, **kwargs) -> List[GithubRepoItem]:
        """
        ...
        for article in li:
            ...
            items.append(
                GithubRepoItem(repo_url=repo, desc=trans_desc, lang=lang, stars=stars, forks=forks,
...
class ArxivDataLoader:
    ...
    def __init__(self):
        self._arxiv_search = functools.partial(arxiv.Search, sort_by=arxiv.SortCriterion.SubmittedDate,
        sort_order=arxiv.SortOrder.Descending, id_list=[])
    
    def mock(self):
        results = []
        for result in self._arxiv_search(id_list=["2403.04642", "2402.17177v2", "2403.03890", "2402.18668", "2303.04137"]).results():
            results.append(
                ArxivPaperItem(
    ...
    def load_data(self, query:List[str], get_latest: bool=False) -> List[ArxivPaperItem]:
        """
        ...
        items = []
        for query_ in query:
            results = self._arxiv_search(query_, max_results=5 if get_latest else 100).results()
            current_date = datetime.datetime.now().date()

            for result in results:
                try:
                    date = datetime.datetime.strptime(str(result.published.date()), '%Y-%m-%d').date()
...
class YoutubeDataLoader:
    ...
    def mock(self):
        ...
        for topic in mock_list:
            results = self.search_by_topic([topic], min_views=1, max_results=1,max_duration_secs=2*60*60)
            mock_results.extend(results)
        return mock_results

    def search_by_topic(self, topics: List[str], min_views: int=1000, max_results: int=100, max_duration_secs: int=1800) -> List[YoutubeTranscriptItem]:
        ...
        for topic in topics:
            ...
            for result in results:
                ...
                num_views = int(result["views"].strip().split("views")[0].strip().replace(",",""))
